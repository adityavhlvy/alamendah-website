<a href="{{ route('main.activity.show', ['id' => $activity['id']]) }}" class="group relative activities rounded-3xl transition-[height] duration-300 overflow-hidden @if(Request::routeIs('main.index')) w-[24rem] h-[24rem] @elseif(Request::routeIs('main.activity')) w-[20rem] h-[20rem] @endif flex-shrink-0" >
    ...
    <div class="relative bg-black w-full h-full bg-opacity-70 rounded-3xl text-white p-5 grid grid-cols-3 grid-rows-3">
        <div id="Forward" class="justify-self-end col-start-3 col-end-4 row-start-1 row-end-2 rounded-full w-[50px] h-[50px] bg-transparent border-4 border-dark-grey flex items-center justify-center">
            <x-svg.forward />
        </div>
        <x-listactivities.forward :id="$activity['id']" />
